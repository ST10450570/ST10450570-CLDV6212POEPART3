    ...
    public class ProductsFunctions
    {
            ...
            [HttpTrigger(AuthorizationLevel.Anonymous, "put", Route = "products/{id}")] HttpRequestData req, string id)
        {
            ...
            try
            {
                ...
                var e = resp.Value;
                string existingImageUrl = e.ImageUrl; // Preserve current image
                ...
                if (form.Text.TryGetValue("Price", out var priceTxt) && double.TryParse(priceTxt, out var price))
                    e.Price = price;
                if (form.Text.TryGetValue("StockAvailable", out var stockTxt) && int.TryParse(stockTxt, out var stock))
                    e.StockAvailable = stock;

                // Handle image
                var imageFile = form.Files.FirstOrDefault(f => f.FieldName == "ImageFile");
                if (imageFile != null && imageFile.Data.Length > 0)
                {
                    var container = new BlobContainerClient(_conn, _images);
                    await container.CreateIfNotExistsAsync();
                    var blob = container.GetBlobClient($"{Guid.NewGuid():N}-{imageFile.FileName}");
                    await using var s = imageFile.Data;
                    await blob.UploadAsync(s, overwrite: false);
                    e.ImageUrl = blob.Uri.ToString(); // Replace with new
                }
                else
                {
                    e.ImageUrl = existingImageUrl; // Keep existing
                }
